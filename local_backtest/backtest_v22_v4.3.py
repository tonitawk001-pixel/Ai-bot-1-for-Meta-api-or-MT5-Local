"""V22 v4.3 — Backtest with 2% flat risk, ADX dynamic TP, execute_trade simulation."""
...
def get_risk_pct(b): return RISK_PERCENT_FLAT / 100.0  # Flat 2%
...
# v4.3: Calculate lot for 2% flat risk
def calc_lot(balance, sl_distance):
    if sl_distance <= 0 or balance <= 0: return 0.01
    risk_amt = balance * 0.02
    risk_per_lot = sl_distance * 100
    raw = risk_amt / risk_per_lot
...
def run_backtest():
    print("=" * 80)
    print("  V22 v4.3 BACKTEST — 2% Flat Risk | ADX Dynamic TP | June 2026")
    print("=" * 80)
    d15, d5 = load_data()
    ...
    for i in range(min(200,len(d15)-1), len(d15)):
        ...
        try:
            rsi_bullish = ind5["rsi"].iloc[-1] > 40 and ind15["rsi"].iloc[-1] > 40
            rsi_bearish = ind5["rsi"].iloc[-1] < 60 and ind15["rsi"].iloc[-1] < 60
            if direction == "BUY" and not rsi_bullish: continue
            if direction == "SELL" and not rsi_bearish: continue
        except: pass
        ...
        # v4.3: Dynamic SL/TP + 2% flat risk lot sizing
    ...
    print(f"  {'#':<4} {'Date':<12} {'Dir':<5} {'Entry':<10} {'Exit':<10} {'P&L':<10} {'Reason':<8} {'Lot':<8} {'Risk%':<6}")
    print(f"  {'-'*72}")
    for idx,t in enumerate(closed,1):
        ...
        sl_dist_pts = abs(t["entry"] - t["sl"])
        risk_pct = (sl_dist_pts * t["lot"] * 100 / STARTING_BALANCE) * 100 if STARTING_BALANCE else 0
        print(f"  {idx:<4} {d:<12} {t['dir']:<5} {t['entry']:<10.2f} {t.get('close_price',0):<10.2f} ${pnl:<+7.2f} {t.get('reason',''):<8} {t['lot']:<8.2f}")

    print(); print("="*80); print(f"  V22 v4.3 PERFORMANCE — {START_DATE[:7]}"); print("="*80)
    ...
    print(f"\n  === V22 v4.3 SPECIFIC STATS ===")
    print(f"  Risk Model:         2% Flat of Balance")
    print(f"  Total entries:      {total_entries}")
    print(f"  Dynamic TP (ADX≥25):{dynamic_tp_count} ({dynamic_tp_count/total_entries*100:.1f}%)" if total_entries else "  Dynamic TP: 0")

    # Compare with v4.1 graduated risk
    avg_risk_pct = 2.0
    print(f"  Risk per trade:     {avg_risk_pct:.1f}% (flat)")
